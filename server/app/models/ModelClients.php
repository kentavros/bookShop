...
class ModelClients extends ModelDB
{
    ...
    {
        if(!preg_match("/^[a-zA-Z0-9]+$/",$param['login']))
        {
            return ERR_LOGIN_NAME;
        }
        if(strlen($param['login']) < 3 || strlen($param['login']) > 30)
        {
            return ERR_LOGIN_LEN;
        }
        $firstName = $this->pdo->quote(trim($param['first_name']));
        $lastName = $this->pdo->quote(trim($param['last_name']));
